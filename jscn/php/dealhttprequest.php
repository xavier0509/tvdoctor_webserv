...
{
  $getTVIDUrl = "http://msg.push.jscnnet.com/api/getClientIdAndPushId";
  #$appid ='nAPkh8JA';
  #$apikey ='sjDG4kZA';
...
function getPushIdByActiveId($appid, $activeId, $accessToken){
  $getPushIDUrl = "http://msg.push.jscnnet.com/api/v3/getPushIdByActiveId";
  $timestamp = microtime_float();
...
function touchPush($id,$appid,$token,$devid){
  $appid = $appid;
  $url = "http://api.touch.push.jscnnet.com/message/pushMsg?channelType=tv&devId=".$devid."&targetId=".$id ."&msg=connect&ttl=120&appId=".$appid."&token=".$token."&targetType=7";
  $result =  httpRequest($url);
  getMsgApi($id,$result);
  $datajson =json_decode($result);
  return $datajson->code;
